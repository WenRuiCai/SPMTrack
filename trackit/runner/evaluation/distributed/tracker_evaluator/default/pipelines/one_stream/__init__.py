...
class OneStreamTracker_Evaluation_MainPipeline(TrackerEvaluationPipeline):
    ...
    def on_tracked(self, model_outputs, context):
        ...
        for index, (task_id, image_size, frame_index) in enumerate(zip(task_ids, x_frame_sizes, x_frame_indices)):
            ...
            # 裁剪当前frame留作memory
            for task_input_data in context.input_data.tasks:
                if task_input_data.id == task_id:
                    tracked_img = task_input_data.tracker_do_tracking_context.input_data['image']
                    template_curation_parameter = get_siamfc_cropping_params(predicted_bounding_box_on_full_search_image, 2.0, np.array([196, 196])) # 2, [196,196]
